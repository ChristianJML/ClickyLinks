...
app.post('/claude-chat', async (req, res) => {
    ...
    try {
        ...
        let claudeMaxTokens = 200; // Default for moderate length
        if (length < 0) {
            claudeMaxTokens = 100; // Shorter responses
        } else if (length > 0) {
            claudeMaxTokens = 300; // Longer responses
        }
...
//         let geminiMaxTokens = 200; // Default for moderate length
//         if (length < 0) {
//             geminiMaxTokens = 100; // Shorter responses
//         } else if (length > 0) {
//             geminiMaxTokens = 300; // Longer responses
//         }

//         const geminiPrompt = systemContent + "\n" + userMessage;
//         console.log('Sending to Gemini:', geminiPrompt);

//         const result = await chat.sendMessage(geminiPrompt, { maxOutputTokens: geminiMaxTokens });
//         const geminiResponse = await result.response;
//         const chatResponse = geminiResponse.text();
